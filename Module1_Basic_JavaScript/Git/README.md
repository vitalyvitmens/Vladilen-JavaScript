...
4. Рекомендуемые типы коммитов:
  ...
  - chore: если не поменялось ничего важного для работы Вашего проекта (поправили орфографические ошибки в комментариях или добавили файлы в .gitignore)
...
4. git stash pop - возвращает наши изменения, вытаскиваем последнюю запись из stash и удаляет из stash последнюю запись, что бы их закомитить после вытаскивания используй команду git add .
5. git stash drop - удалить все записи из stash
6. git stash apply - применить изменения не удаляя изменения из stash
7. git stash apply stash@{0} - применить нулевую запись находящуюся в stash, перед этим посмотри идентификатор нужной записи (stash@{0}) при помощи команды git stash list
...
6. коллега исправляет и коммитит эти изменения и говорит забирай этот фикс к себе передает хэш коммита [SHA], я волью эти изменения позже, нам нужно влить именно эти изменения
